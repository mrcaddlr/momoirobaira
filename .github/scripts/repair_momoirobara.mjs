...
if (fs.existsSync(EXTERNAL)) feature = fs.readFileSync(EXTERNAL, 'utf8').trim();
...
if (!html.includes('MOMOIROBARA INLINE CLEANUP')) html = html.replace(/<\/style>/i, css + '</style>');
...
console.log(feature ? 'feature layer inlined' : 'no feature layer found');
