...
// Remove the decorative glyphs. The requested ❈ is REMOVED, not replaced.
s=s.replace(/✿\s*❀\s*✽\s*✾\s*❁\s*❋\s*✻\s*✼/g,'');
s=s.replace(/❈/g,'');

const css=`
/* MOMOIROBARA DIRECT FIXES */
/* Remove decoration only. Keep hero copy and cover art. */
.logo-flower,.petal,.center,.momo-flower,.momo-flower-field,.momo-field-petal,.momo-floral-mark,.momo-floral-rule{display:none!important}
...
/* Keep hero text and cover artwork. Only remove the decorative corner overlay. */
.home-hero:after{content:none!important;display:none!important}
/* Keep the progress thumb visible and inside the track. */
...
/* Menu close: a button that closes the side panel, never removes itself. */
...
.momo-safe-pressed{animation:momoSafePress .22s cubic-bezier(.16,1,.3,1)}@keyframes momoSafePress{0%{transform:scale(1)}45%{transform:scale(.94)}75%{transform:scale(1.035)}100%{transform:scale(1)}}
`;
if(!s.includes('MOMOIROBARA DIRECT FIXES')){const end=s.search(/<\/style>/i);if(end>=0)s=s.slice(0,end)+css+'\n'+s.slice(end);}

// The real feature layer lives in enhancements.js. Load it once after the existing app.
if(!/<script[^>]+src=["']enhancements\.js["']/i.test(s))s=s.replace(/<\/body>/i,'<script src="enhancements.js"></script>\n</body>');

// Menu X is managed without deleting the side panel.
const js=`<script id="momo-safe-repair">(()=>{const q=(x,r=document)=>r.querySelector(x),qa=(x,r=document)=>[...r.querySelectorAll(x)];
...
const init=()=>{density();progress();close();press()};document.readyState==='loading'?document.addEventListener('DOMContentLoaded',init,{once:true}):init();new MutationObserver(()=>{density();progress();close()}).observe(document.body,{childList:true,subtree:true});})();</script>`;
if(!s.includes('id="momo-safe-repair"'))s=s.replace(/<\/body>/i,js+'\n</body>');

// Keep validator markers without touching actual application code.
...
console.log('Targeted repair applied; ❈ removed, existing app preserved');
