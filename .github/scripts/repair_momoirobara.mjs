...
try {
  ...
  for (const commit of commits) {
    try {
      const candidate = execFileSync('git', ['show', `${commit}:${EXTERNAL}`], { encoding: 'utf8' }).trim();
      if (candidate) { feature = candidate; break; }
    } catch {}
  }
} catch {}

if (fs.existsSync(EXTERNAL)) {
  const candidate = fs.readFileSync(EXTERNAL, 'utf8').trim();
...
if (!html.includes('MOMOIROBARA INLINE CLEANUP')) {
...
console.log(feature ? 'feature layer inlined' : 'no historical feature layer found');
